...
const listGeneralHabits = /* GraphQL */ `query ListGeneralHabits {
  listGeneralHabits {
    items {
      id
      name
...
const updateGeneralHabit = /* GraphQL */ `mutation UpdateGeneralHabit($input: UpdateGeneralHabitInput!) {
  updateGeneralHabit(input: $input) {
    id
    name
    type
    completed
...
export const handler = async (event) => {
  ...
  const listHabitsRequestToBeSigned = new HttpRequest({
    ...
    body: JSON.stringify({ query: listGeneralHabits }),
  ...
  try {
    ...
    let habits = body.data.listGeneralHabits.items
    habits.forEach(async (object) => {
      ...
      const updateHabitsRequestToBeSigned = new HttpRequest({
        ...
        hostname: endpoint.host,
        body: JSON.stringify({ query: updateGeneralHabit, variables }),
        path: endpoint.pathname
      });
